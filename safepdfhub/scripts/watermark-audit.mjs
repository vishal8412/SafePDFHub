...
const studioDialog = await read('src/app/features/studio/watermark/studio-watermark-dialog.component.ts');
...
assert(controlsHtml.includes('All pages') && controlsHtml.includes('Page range'), 'Page-scope controls are missing.');
...
assert(studioShellHtml.includes('app-studio-watermark-dialog'), 'Studio Watermark dialog is not wired into the shell.');
assert(studioDialog.includes('WatermarkControlsComponent'), 'Studio must reuse the shared Watermark controls.');
assert(studioShell.includes('exportCurrentDocumentFile'), 'Studio must export current Studio changes before watermarking.');
...
console.log('- Multi-page preview navigation and all/range page scope detected.');
